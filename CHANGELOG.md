# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [6.7.46] - 2026-07-16

### Fixed

- Correct logging rules separator from comma to semicolon

## [6.7.45] - 2026-07-09

## [6.7.44] - 2026-06-17

### Changed

- Adjust default logging level to disable info output

## [6.7.43] - 2026-06-04

### Added

- Add disableInWindowBlur config to org.deepin.dtk.preference

### Changed

- Chore: New release 6.7.36
- Chore: New release 6.7.37
- Chore: New release 6.7.38
- Chore: New release 6.7.39
- Chore: New release 6.7.40
- New release 6.7.41 (#140)
- Chore: New release 6.7.42

## [6.7.42] - 2026-05-20

### Changed

- Chore: New release 6.7.36
- Chore: New release 6.7.37
- Chore: New release 6.7.38
- Chore: New release 6.7.39
- Chore: New release 6.7.40
- New release 6.7.41 (#140)

## [6.7.41] - 2026-05-08

### Changed

- Chore: New release 6.7.36
- Chore: New release 6.7.37
- Chore: New release 6.7.38
- Chore: New release 6.7.39
- Chore: New release 6.7.40

## [6.7.40] - 2026-04-23

### Changed

- Chore: New release 6.7.36
- Chore: New release 6.7.37
- Chore: New release 6.7.38
- Chore: New release 6.7.39

## [6.7.39] - 2026-04-09

### Changed

- Chore: New release 6.7.36
- Chore: New release 6.7.37
- Chore: New release 6.7.38

## [6.7.38] - 2026-03-31

### Changed

- Chore: New release 6.7.36
- Chore: New release 6.7.37

## [6.7.37] - 2026-03-25

### Changed

- Chore: New release 6.7.36

## [6.7.36] - 2026-03-12

## [6.7.35] - 2026-03-05

### Changed

- Chore: New release 6.7.33
- Chore: New release 6.7.34

## [6.7.34] - 2026-02-27

### Changed

- Chore: New release 6.7.33

## [6.7.33] - 2026-02-05

## [6.7.32] - 2026-01-22

## [6.7.31] - 2026-01-06

## [5.7.30] - 2025-12-11

## [5.7.28] - 2025-12-04

## [5.7.27] - 2025-11-27

### Fixed

- Update dtk preference logging rules configuration

## [5.7.26] - 2025-11-06

### Added

- Add default color mode configuration for print preview

### Changed

- Chore: New release 5.7.25
- Update debian copyright license from GPL-3+ to BSD-3-Clause

## [5.7.25] - 2025-10-30

## [5.7.24] - 2025-10-16

## [5.7.23] - 2025-09-25

## [5.7.22] - 2025-09-04

## [5.7.21] - 2025-08-14

## [5.7.20] - 2025-07-31

### Added

- Add debian check workflow

## [5.7.19] - 2025-07-03

## [5.7.18] - 2025-06-27

### Fixed

- Update default logging rules in dtk preference
- Make version optional in release workflow and update RPM spec

## [5.7.17] - 2025-06-19

### Changed

- Support VERSION file (#102)
- Update debian/control

[5.7.17]: https://github.com/linuxdeepin/dtkcommon/compare/5.7.16..5.7.17

<!-- generated by git-cliff -->
